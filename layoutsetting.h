#ifndef LAYOUTSETTING_H
#define LAYOUTSETTING_H

namespace layout {

// DO NOT CHANGE THIS! THIS IS FIXED!
constexpr int RIGHT_W             = 168;
constexpr int RIGHT_HEADER_H      = 48;
constexpr int RIGHT_LEFTBORDER_W  = 12;
constexpr int RIGHT_RIGHTBORDER_W = 15;
constexpr int RIGHT_RIGHTBORDER_X = 153;
constexpr int RIGHT_BORDER_H      = 110;
constexpr int RIGHT_MAPNAME_H     = 12;

constexpr int MAP_W = 142;
constexpr int MAP_H = 111;

constexpr int TOP_W            = 1072;
constexpr int TOP_H            = 96;
constexpr int TOP_COLOR_BG_H   = 76;  // 玩家颜色背景图片高度
constexpr int TOP_PLAYERNAME_Y = 18;
constexpr int TOP_COUNTRY_X    = 280;
constexpr int TOP_COUNTRY_Y    = 30;
constexpr int TOP_COUNTRY_W    = 30;
constexpr int TOP_COUNTRY_H    = 20;
constexpr int TOP_I_X          = 320;
constexpr int TOP_I_WH         = 20;
constexpr int TOP_IBALANCE_Y   = 18;
constexpr int TOP_IPOWER_Y     = 44;
constexpr int TOP_BALANCE_X    = 346;
constexpr int TOP_BALANCE_Y    = 20;
constexpr int TOP_BALANCE_W    = 61;
constexpr int TOP_BALANCE_H    = 18;
constexpr int TOP_POWER_X      = 347;
constexpr int TOP_POWER_Y      = 47;
constexpr int TOP_POWER_W      = 30;
constexpr int TOP_POWER_H      = 12;

constexpr int IF_BAR_H = 16;

constexpr int BOTTOM_H        = 32;
constexpr int BOTTOM_HS       = 14;
constexpr int BOTTOM_FILL_H   = 12;
constexpr int BOTTOM_CREDIT_X = 10;
constexpr int BOTTOM_CREDIT_W = 50;
constexpr int BOTTOM_CREDIT_H = 14;

constexpr int UNIT_W    = 69;
constexpr int UNIT_WS   = 80;  
constexpr int UNIT_H    = 55;
constexpr int UNIT_HS   = 62;  // Increased from 61 to 62, adding 1px between unit blocks
constexpr int UNIT_BG_H = 16;

constexpr int UNITBLOCK_Y = 32;

// Only define base values for producing blocks - actual dimensions are calculated in globalsetting.cpp
constexpr int PRODUCINGBLOCK_X     = 10;
constexpr int PRODUCINGBLOCK_Y     = 180;
constexpr int PRODUCINGBLOCK_SPACE = 5;   // 水平间距
constexpr int PRODUCINGBLOCK_VSPACE = 20; // 垂直间距，增加到25使两个玩家的建造栏间距更大
constexpr int PRODUCING_IMG_X      = 1;
constexpr int PRODUCING_IMG_Y      = 1;
constexpr int PRODUCING_IMG_W      = 70;
constexpr int PRODUCING_IMG_H      = 55;
constexpr int PRODUCING_STATUS_Y   = 10;
constexpr int PRODUCING_NUMBER_Y   = 39;
constexpr int PRODUCING_PROGRESS_W = 70;
constexpr int PRODUCING_PROGRESS_H = 7;

// Font
constexpr char OPPO_B[] = "OPlusSans 3.0";
constexpr char OPPO_M[] = "OPlusSans 3.0 Medium";

// Color
constexpr char COLOR_DEFAULT[] = "262728";

};  // end of namespace layout

#endif  // LAYOUTSETTING_H
