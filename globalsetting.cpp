...
void Globalsetting::loadLayoutSetting(int width, int height, float ratio) {
    ...
    l.top_m               = l.right_x / 2;
    l.top_w               = layout::TOP_W;
    l.top_h               = layout::TOP_H;
    ...
    l.unit_y              = layout::RIGHT_HEADER_H * ratio + layout::MAP_H * ratio + (6 + 80) * ratio; // 下移8*ratio像素
    ...
    l.producingblock_y2    = l.producingblock_y1 + l.producingblock_h + layout::PRODUCINGBLOCK_VSPACE; // 使用垂直间距常量
...
QString Globalsetting::findNameByNickname(const QString &nickname) {
    // 优先从本地缓存中查找，这个缓存包含远程获取的数据
    if (playerMapping.contains(nickname)) {
        return playerMapping[nickname];
    }

    // 如果本地缓存中没有，再尝试从已加载的玩家列表中查找（保持向后兼容）
    for (const auto &player : player_list) {
        if (player.isObject()) {
            QJsonObject obj = player.toObject();
            if (obj["nickname"].toString() == nickname) {
                QString name = obj["playername"].toString();
                // 将找到的结果也添加到缓存中
                playerMapping[nickname] = name;
                return name;
            }
        }
    }
